#!/usr/bin/env python3
"""
Standalone Phase 1 Sports Analysis Tool

This tool implements Phase 1 features for comprehensive sports photo analysis
without dependencies on the main project's Pydantic settings.
...
# Import our new Phase 1 modules directly
from src.soccer_photo_sorter.detectors.ball_detector import BallDetector, BallTrackingResult
from src.soccer_photo_sorter.detectors.action_classifier import ActionClassifier, ActionClassificationResult
from src.soccer_photo_sorter.detectors.field_analyzer import FieldAnalyzer, FieldAnalysisResult
from src.soccer_photo_sorter.detectors.quality_assessor import QualityAssessor, QualityAssessment


@dataclass
class StandalonePhase1Result:
    """Standalone Phase 1 analysis result for an image."""
    image_path: str
...
@dataclass
class StandalonePhase1Summary:
    """Summary of standalone Phase 1 analysis."""
    total_images: int
...
class StandalonePhase1Analyzer:
    """Standalone Phase 1 sports analysis system."""
    ...
    def __init__(self, 
        ...
        """
        Initialize the standalone Phase 1 analyzer.
        ...
        # Initialize OpenCV face detection for player detection
        ...
        logger.info("Initialized StandalonePhase1Analyzer")
    
    def analyze_image(self, image_path: Path) -> StandalonePhase1Result:
        """
        ...
        try:
            ...
            pose_count = self._detect_poses(image)
            
            # Extract player bounding boxes from face detections
            player_bboxes = self._extract_player_bboxes(image)
    ...
    def _detect_poses(self, image: np.ndarray) -> int:
    ...
    def _extract_player_bboxes(self, image: np.ndarray) -> List[Tuple[int, int, int, int]]:
        """Extract player bounding boxes from face detections."""
        try:
            gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            faces = self.face_cascade.detectMultiScale(gray, 1.1, 4)
            
            # Convert face detections to player bounding boxes
            player_bboxes = []
            for (x, y, w, h) in faces:
                # Expand face bounding box to include more of the player
                expanded_x = max(0, x - w//2)
                expanded_y = max(0, y - h//2)
                expanded_w = min(image.shape[1] - expanded_x, w * 2)
                expanded_h = min(image.shape[0] - expanded_y, h * 3)
                
                player_bboxes.append((expanded_x, expanded_y, expanded_w, expanded_h))
            
            return player_bboxes
        except Exception as e:
            logger.debug(f"Player bbox extraction failed: {e}")
            return []
    
    def analyze_images(self, image_pattern: str, output_dir: Path, max_images: Optional[int] = None) -> List[StandalonePhase1Result]:
        """
    ...
    def _save_annotated_image(self, result: StandalonePhase1Result, output_dir: Path) -> None:
    ...
    def create_summary(self, results: List[StandalonePhase1Result], output_dir: Path) -> StandalonePhase1Summary:
        ...
        summary = StandalonePhase1Summary(
        ...
        summary_path = output_dir / "standalone_phase1_summary.json"
    ...
    def _create_markdown_report(self, summary: StandalonePhase1Summary, results: List[StandalonePhase1Result], output_dir: Path) -> None:
        """Create comprehensive markdown report."""
        report_path = output_dir / "standalone_phase1_report.md"
        
        with open(report_path, 'w') as f:
            f.write("# Standalone Phase 1 Sports Analysis Report\n\n")
        ...
        logger.info(f"Standalone Phase 1 analysis report saved to {report_path}")
...
def main(input_pattern: str, output_dir: str, max_images: Optional[int], 
    ...
    """Perform comprehensive standalone Phase 1 sports analysis."""
    
    # Setup logging
    if verbose:
        logger.add("standalone_phase1_analysis.log", level="DEBUG")
    
    # Create output directory
    output_path = Path(output_dir)
    
    # Initialize analyzer
    analyzer = StandalonePhase1Analyzer(
    ...
    # Analyze images
    logger.info("Starting standalone Phase 1 sports analysis")
    ...
    # Print summary
    logger.info("Standalone Phase 1 analysis complete!")
