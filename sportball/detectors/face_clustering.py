...
class FaceClustering:
    ...
    def cluster_faces_from_detections(
        self, detection_results: Dict[str, Any], max_faces: Optional[int] = None
        ...
        try:
            ...
            face_encodings, face_metadata = self._extract_face_data(
                detection_results, max_faces
            )
            ...
            if len(face_encodings) > 1:
                ...
                if all(
                    np.array_equal(encoding, first_encoding)
                    for encoding in face_encodings[1:]
                ):
                    self.logger.warning(
                        "All face encodings are identical - no meaningful clustering possible"
                    )
                    return FaceClusteringResult(
                        clusters=[],
                        unclustered_faces=[
                            metadata["face_id"] for metadata in face_metadata
                        ],
            ...
            self.logger.info(
                f"Clustering completed: {len(clusters)} clusters, {len(unclustered_faces)} unclustered faces"
            )
    ...
    def cluster_faces_from_directory(
        self, directory: Path, pattern: str = "*.json", max_faces: Optional[int] = None
    ) -> FaceClusteringResult:
        """
                ...
                try:
                    with open(sidecar_file, "r") as f:
                        ...
                        if "face_detection" in data and data["face_detection"].get(
                            ...
                            if self.verbose:
                                self.logger.debug(
                                    f"Loaded face detection data for {image_path}: {len(data['face_detection'].get('faces', []))} faces"
                                )
                except Exception as e:
                    self.logger.warning(
                        f"Failed to load sidecar file {sidecar_file}: {e}"
                    )

            if self.verbose:
                self.logger.info(
                    f"Loaded detection results for {len(detection_results)} images"
                )
        ...
        for image_path, detection_data in detection_results.items():
            ...
            for face_idx, face_data in enumerate(faces):
                ...
                if "encoding" not in face_data or face_data["encoding"] is None:
                    if self.verbose:
                        self.logger.debug(
                            f"Skipping face {face_idx} in {image_path}: no encoding"
                        )
                    continue
                ...
                if self.verbose:
                    self.logger.debug(
                        f"Added encoding for face {face_idx} in {image_path}: shape {encoding.shape}"
                    )
    ...
    def _dbscan_clustering(self, X: np.ndarray) -> np.ndarray:
        ...
        clustering = DBSCAN(eps=eps, min_samples=self.min_cluster_size, metric="cosine")
    ...
    def _group_faces_into_clusters(
        self,
    ...
    def export_clusters(
        ...
        try:
            ...
            export_results = {"success": True, "files_created": [], "error": None}
            ...
            return {"success": False, "files_created": [], "error": str(e)}

    def _export_clusters_csv(
        self, clustering_result: FaceClusteringResult, csv_file: Path
    ):
        ...
        with open(csv_file, "w", newline="") as f:
            ...
            for cluster in clustering_result.clusters:
                for i, face_id in enumerate(cluster.face_ids):
                    image_path = (
                        cluster.image_paths[i] if i < len(cluster.image_paths) else ""
                    )
                    confidence = (
                        cluster.confidence_scores[i]
                        if i < len(cluster.confidence_scores)
                        else 0.0
                    )
                    ...
                    writer.writerow(
                        [
                            ...
                            clustering_result.parameters.get(
    ...
    def visualize_clusters(
        ...
        if not CV2_AVAILABLE:
            return {"success": False, "error": "OpenCV not available for visualization"}
        ...
        try:
            ...
            return {"success": False, "images_created": [], "error": str(e)}
