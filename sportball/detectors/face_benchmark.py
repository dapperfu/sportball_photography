...
from .face import FaceDetector, InsightFaceDetector, FaceDetectionResult, DetectedFace
...
class FaceDetectionBenchmark:
    ...
    def benchmark_single_image(self, 
                             image_path: Path,
                             detectors: Optional[List[str]] = None) -> Dict[str, BenchmarkResult]:
        """
        ...
        for detector_name in detectors:
            ...
            try:
                ...
                self.logger.error(f"Benchmark failed for {detector_name} on {image_path}: {e}")
    ...
    def benchmark_batch(self, 
                       ...
                       max_images: Optional[int] = None) -> BenchmarkSummary:
        """
        ...
        self.logger.info(f"Starting benchmark on {len(image_paths)} images with {len(detectors)} detectors")
    ...
    def _calculate_performance_stats(self, results: Dict[str, List[BenchmarkResult]]) -> Dict[str, Dict[str, float]]:
        ...
        for detector_name, detector_results in results.items():
            ...
            avg_faces_per_image = total_faces / len(successful_results) if successful_results else 0
    ...
    def _calculate_accuracy_stats(self, results: Dict[str, List[BenchmarkResult]]) -> Dict[str, Dict[str, float]]:
    ...
    def save_benchmark_results(self, 
        ...
        try:
            ...
            for detector_name, detector_results in summary.results.items():
                results_dict[detector_name] = [asdict(result) for result in detector_results]
    ...
    def compare_detectors(self, summary: BenchmarkSummary) -> Dict[str, Any]:
        ...
        if fastest_detector:
            recommendations.append(f"Fastest detector: {fastest_detector} ({fastest_time:.3f}s avg)")
        
        if most_accurate_detector:
            recommendations.append(f"Most accurate detector: {most_accurate_detector} ({highest_confidence:.3f} avg confidence)")
        
        if most_reliable_detector:
            recommendations.append(f"Most reliable detector: {most_reliable_detector} ({highest_success_rate:.1%} success rate)")
        ...
        for detector_name in summary.detectors:
            if detector_name in summary.performance_stats:
                ...
                comparison['detailed_comparison'][detector_name] = {
                    ...
                    'accuracy': acc_stats,
                    'overall_score': self._calculate_overall_score(perf_stats, acc_stats)
                }
    ...
    def _calculate_overall_score(self, perf_stats: Dict[str, float], acc_stats: Dict[str, float]) -> float:
        """
        ...
        speed_score = max(0, 100 - (perf_stats['avg_time'] * 100))  # Lower time = higher score
        reliability_score = perf_stats['success_rate'] * 100  # Higher success rate = higher score
        accuracy_score = acc_stats.get('avg_confidence', 0) * 100  # Higher confidence = higher score
        
        # Weighted average (40% speed, 30% reliability, 30% accuracy)
        overall_score = (speed_score * 0.4 + reliability_score * 0.3 + accuracy_score * 0.3)
