...
class ObjectDetector:
    ...
    def __init__(
        ...
        self.class_name_to_id = {
            name: class_id for class_id, name in COCO_CLASSES.items()
        }
        ...
        logger.info(
            f"Initialized ObjectDetector with {border_padding * 100:.0f}% border padding, GPU: {self.enable_gpu}"
        )
    ...
    def _detect_single_image(
        self, image_path: Path, force: bool = False, save_sidecar: bool = True
    ) -> DetectionResult:
    ...
    def _detect_multiple_images(
        ...
        if self.enable_gpu and self.device == "cuda":
            ...
            return self._detect_multiple_images_gpu_batch(
                image_paths, force, save_sidecar
            )
        else:
    ...
    def _detect_multiple_images_gpu_batch(
        self, image_paths: List[Path], force: bool, save_sidecar: bool = True
    ) -> List[DetectionResult]:
    ...
    def _detect_multiple_images_cpu_parallel(
        self,
        image_paths: List[Path],
        force: bool,
        max_workers: Optional[int],
    ...
    def _calculate_optimal_batch_size(self) -> int:
        ...
        try:
            ...
            logger.info(
                f"GPU memory: {gpu_memory_gb:.1f} GB, estimated {memory_per_image_mb} MB per image"
            )
            ...
            logger.warning(
                f"Failed to calculate optimal batch size: {e}, using default: {self.gpu_batch_size}"
            )
            return self.gpu_batch_size

    def _process_gpu_batch(
        self, image_paths: List[Path], force: bool
    ) -> List[DetectionResult]:
        ...
        try:
            ...
            for i, (result, metadata) in enumerate(zip(batch_results, batch_metadata)):
                ...
                if boxes is not None:
                    for j, box in enumerate(boxes):
                        ...
                        crop_width_percent = min(
                            1.0 - crop_x_percent, width_percent + 2 * padding_x_percent
                        )
                        crop_height_percent = min(
                            1.0 - crop_y_percent, height_percent + 2 * padding_y_percent
                        )
                ...
                detection_result = DetectionResult(
                    ...
                    / len(batch_images),  # Average time per image
            ...
            for result in results:
                ...
                if boxes is not None:
                    for i, box in enumerate(boxes):
                        ...
                        detected_objects.append(detected_object)
                        logger.debug(
                            f"Detected {class_name}: {orig_w}x{orig_h} at ({orig_x1}, {orig_y1}), confidence: {confidence:.3f}"
                        )
        ...
        **kwargs,
        ...
        if not objects:
            return {
                "success": True,
                "objects_extracted": 0,
                "message": "No objects match filter criteria",
            }
        ...
        for i, obj_data in enumerate(objects):
            try:
                ...
                object_filename = (
                    f"{image_path.stem}_{class_name}_{class_counter:02d}.jpg"
                )
                ...
                if create_individual and crop_width > 0 and crop_height > 0:
                    ...
                    # Save individual object with EXIF preservation
                    pil_object_image = Image.fromarray(object_image)
                ...
                if create_annotated:
                    ...
                    # Draw annotation
                    image = self._draw_annotation(
                        image, x, y, width, height, label, color
                    )
        ...
        if create_annotated and objects:
            ...
            # Save annotated image with EXIF preservation
            pil_annotated_image = Image.fromarray(image)
    ...
    def _draw_annotation(
        ...
        label_x = x
        label_y = y - text_height - 10 if y - text_height - 10 > 0 else y + height + 10
    ...
    def _annotate_individual_object(
        self, object_image: np.ndarray, class_name: str, confidence: float
    ) -> np.ndarray:
        """Add annotation to an individual extracted object using PIL."""
        # Convert numpy array to PIL Image
        ...
        img_height, img_width = object_image.shape[:2]
        ...
        draw.rectangle(
            [label_x, label_y - text_height, label_x + text_width, label_y],
            fill=(0, 0, 0),
        )  # Black background

        # Draw label text
        draw.text(
            (label_x, label_y - text_height), label, fill=(255, 255, 255), font=font
        )
    ...
    def _save_sidecar(self, detection_result: DetectionResult) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to save sidecar for {detection_result.image_path}: {e}"
            )
            return False
