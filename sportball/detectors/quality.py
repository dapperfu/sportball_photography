...
class QualityAssessor:
    ...
    def __init__(self, cache_enabled: bool = True):
        ...
        self.thresholds = {
            ...
            "contrast": {"excellent": 0.7, "good": 0.5, "fair": 0.3, "poor": 0.2},
        }

        self.logger.info("Initialized QualityAssessor")

    def assess_quality(self, image_path: Union[Path, str], **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"error": str(e), "success": False, "image_path": str(image_path)}
    ...
    def _assess_focus(self, image: np.ndarray) -> float:
        ...
        scores = {
            ...
            "var_of_laplacian": min(
                1.0, variance_of_laplacian / (0.01 * image_size_factor)
            ),
            "modified_laplacian": min(
                1.0, modified_laplacian / (1000 * image_size_factor)
            ),
        }
    ...
    def _assess_exposure(self, image: np.ndarray) -> float:
        ...
        if thresholds["optimal_min"] <= mean_brightness <= thresholds["optimal_max"]:
            ...
            thresholds["acceptable_min"]
            <= mean_brightness
            <= thresholds["acceptable_max"]
        ):
            return 0.8
        else:
            # Calculate penalty for extreme values
            if mean_brightness < thresholds["acceptable_min"]:
                penalty = (thresholds["acceptable_min"] - mean_brightness) / thresholds[
                    "acceptable_min"
                ]
            else:
                penalty = (mean_brightness - thresholds["acceptable_max"]) / (
                    255 - thresholds["acceptable_max"]
                )
    ...
    def _generate_recommendations(self, metrics: QualityMetrics) -> List[str]:
        ...
        if metrics.sharpness_score < 0.5:
            recommendations.append(
                "Consider using faster shutter speed or better focus technique"
            )

        if metrics.focus_score < 0.5:
            recommendations.append(
                "Image appears out of focus - check autofocus settings or manual focus"
            )

        if metrics.exposure_score < 0.5:
            recommendations.append(
                "Adjust exposure settings - image may be over/under exposed"
            )

        if metrics.composition_score < 0.5:
            recommendations.append(
                "Consider applying rule of thirds for better composition"
            )
