...
class RustSidecarManager:
    ...
    def _check_rust_availability(self) -> None:
        ...
        try:
            result = subprocess.run(['which', 'sportball-sidecar-rust'], 
        ...
        if rust_binary.exists():
            ...
            self.rust_available = True
            self.logger.info(f"Rust binary found in parent directory: {self.config.rust_binary_path}")
            return
    ...
    def validate_sidecars(self, directory: Path, 
    ...
    def export_sidecar_data(self, directory: Path, output_path: Path,
        ...
        if not self.rust_available:
            if self.config.fallback_to_python:
                return self._python_export_data(directory, output_path, operation_filter, format)
            else:
                raise RuntimeError("Rust not available and fallback disabled")
    ...
    def _python_validate_sidecars(self, directory: Path, 
                                 operation_filter: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def _python_get_statistics(self, directory: Path, 
                              operation_filter: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _python_export_data(self, directory: Path, output_path: Path,
                           operation_filter: Optional[str] = None,
                           format: str = "json") -> int:
        ...
        export_data = {
            'exported_at': json.dumps({'$date': {'$numberLong': str(int(__import__('time').time() * 1000))}}),
