...
class ParallelJSONValidator:
    ...
    def validate_json_file(
        self, file_path: Path, validation_rules: Optional[List[Callable]] = None
    ) -> ValidationResult:
        """
    ...
    def validate_json_files_parallel(
        self,
        ...
        self.logger.info(
            f"Validating {len(file_paths)} JSON files with {self.max_workers} workers"
        )
        ...
        with executor_class(max_workers=self.max_workers) as executor:
            ...
            future_to_path = {
                executor.submit(
                    self.validate_json_file, file_path, validation_rules
                ): file_path
            ...
            if show_progress:
                progress_bar = tqdm(
                    total=len(file_paths), desc="Validating JSON files", unit="files"
                )
        ...
        self.logger.info(
            f"Validation completed in {total_time:.2f}s ({len(file_paths) / total_time:.1f} files/sec)"
        )
    ...
    def validate_sidecar_files(
        ...
        return self.validate_json_files_parallel(
            json_files, show_progress=show_progress
        )

    def validate_detection_results(
        self, results: Dict[str, DetectionResult], show_progress: bool = True
    ) -> List[ValidationResult]:
        """
        ...
        if show_progress:
            try:
                ...
                progress_bar = tqdm(
                    total=len(results),
                    desc="Validating detection results",
                    unit="results",
                )
            except ImportError:
                progress_bar = None
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            "valid_percentage": (self.stats["valid_files"] / self.stats["total_files"])
            * 100,
            "invalid_percentage": (
                self.stats["invalid_files"] / self.stats["total_files"]
            )
            * 100,
