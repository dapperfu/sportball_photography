...
def assess(
    ...
    _get_console().print(
        f"📸 Assessing quality of {len(image_paths)} images...", style="blue"
    )
...
def display_quality_results(
    results: dict, filter_low_quality: bool, output_dir: Optional[Path]
):
...
def filter(
    ...
    _get_console().print(
        f"🔍 Filtering images by quality score >= {min_score}...", style="blue"
    )
...
@click.pass_context
def report(
    ctx: click.Context, input_path: Path, output: Optional[Path], report_format: str
):
    """
    ...
    _get_console().print(
        f"📊 Generating quality report for {input_path}...", style="blue"
    )
