...
def sidecar_summary(ctx: click.Context, directory: Path, operation: Optional[str]):
    ...
    for op_type, count in sorted(summary.items(), key=lambda x: x[1], reverse=True):
        ...
        table.add_row(
            op_type.replace("_", " ").title(), str(count), f"{percentage:.1f}%"
        )
...
def cleanup_sidecars(ctx: click.Context, directory: Path, dry_run: bool):
    ...
    if dry_run:
        _get_console().print(
            f"🔍 Scanning for orphaned sidecar files in {directory}...", style="blue"
        )
        # TODO: Implement dry run mode
        _get_console().print("Dry run mode not yet implemented", style="yellow")
    else:
        _get_console().print(
            f"🧹 Cleaning up orphaned sidecar files in {directory}...", style="blue"
        )
        ...
        if removed_count > 0:
            _get_console().print(
                f"✅ Removed {removed_count} orphaned sidecar files", style="green"
            )
        else:
...
@click.pass_context
def delete_sidecars(
    ctx: click.Context, directory: Path, operation: Optional[str], dry_run: bool
):
    """
    ...
    if dry_run:
        _get_console().print(
            f"🔍 Scanning for sidecar files to delete in {directory}...", style="blue"
        )
        # TODO: Implement dry run mode
        _get_console().print("Dry run mode not yet implemented", style="yellow")
    else:
        _get_console().print(
            f"🗑️  Deleting sidecar files in {directory}...", style="blue"
        )
...
def gpu_check(ctx: click.Context):
    ...
    try:
        ...
        if torch.cuda.is_available():
            gpu_table.add_row(
                "CUDA Support", "✅ Available", f"Version {torch.version.cuda}"
            )
            ...
            for i in range(device_count):
                ...
                gpu_table.add_row(
                    f"GPU {i}", "✅ Active", f"{gpu_name} ({gpu_memory_gb:.1f} GB)"
                )
            ...
            try:
                ...
                gpu_table.add_row(
                    "GPU Operations", "✅ Working", "Tensor operations successful"
                )
                ...
                gpu_table.add_row(
                    "GPU Operations", "❌ Failed", f"Error: {str(e)[:50]}..."
                )
        ...
        gpu_table.add_row(
            "CUDA Support", "❌ Not Available", "PyTorch required for CUDA"
        )
    ...
    try:
        ...
        try:
            ...
            if cuda_devices > 0:
                gpu_table.add_row(
                    "OpenCV GPU", "✅ Available", f"{cuda_devices} CUDA device(s)"
                )
                ...
                try:
                    ...
                    gpu_table.add_row(
                        "OpenCV CUDA Ops",
                        "✅ Working",
                        "GPU matrix operations successful",
                    )
                except Exception as e:
                    gpu_table.add_row(
                        "OpenCV CUDA Ops",
                        "❌ Failed",
                        f"GPU operations failed: {str(e)[:30]}...",
                    )
            else:
                gpu_table.add_row(
                    "OpenCV GPU", "❌ Not Available", "OpenCV compiled without CUDA"
                )
        except AttributeError:
            gpu_table.add_row(
                "OpenCV GPU", "❌ Not Available", "OpenCV compiled without CUDA support"
            )
    except ImportError:
            ...
            if torch.cuda.is_available():
                try:
                    ...
                    if "cuda" in device_info:
                        gpu_table.add_row(
                            "YOLO GPU", "✅ Available", f"YOLO using GPU: {device_info}"
                        )
                    else:
                        gpu_table.add_row(
                            "YOLO GPU", "⚠️  Limited", f"YOLO device: {device_info}"
                        )
                except Exception as e:
                    gpu_table.add_row(
                        "YOLO GPU",
                        "❌ Failed",
                        f"Failed to move to GPU: {str(e)[:30]}...",
                    )
            else:
                gpu_table.add_row(
                    "YOLO GPU", "⚠️  Limited", "CUDA not available for YOLO"
                )
        except Exception as e:
            gpu_table.add_row(
                "YOLO GPU", "❌ Error", f"Model loading failed: {str(e)[:30]}..."
            )
    except ImportError:
    ...
    try:
        ...
        gpu_table.add_row(
            "Face Recognition", "✅ Available", "Face recognition library loaded"
        )
        ...
        gpu_table.add_row(
            "Face Recognition", "❌ Not Installed", "Face recognition not available"
        )
    ...
    if core.enable_gpu:
        _get_console().print(
            "✅ GPU acceleration is enabled in sportball configuration", style="green"
        )
    else:
        _get_console().print(
            "⚠️  GPU acceleration is disabled in sportball configuration", style="yellow"
        )
        _get_console().print(
            "   Use --gpu flag to enable GPU acceleration", style="yellow"
        )
        ...
        if torch.cuda.is_available():
            ...
            for i in range(torch.cuda.device_count()):
                ...
                if props.total_memory < 2 * 1024**3:  # Less than 2GB
                    _get_console().print(
                        f"⚠️  GPU {i} has limited memory ({props.total_memory / (1024**3):.1f} GB)",
                        style="yellow",
                    )
                    _get_console().print(
    ...
    _get_console().print(
        "• Install CUDA-enabled PyTorch: pip install torch[cuda]", style="white"
    )
    _get_console().print(
        "• OpenCV GPU support requires building from source with CUDA", style="white"
    )
    _get_console().print(
        "• For full GPU acceleration, build OpenCV with CUDA support", style="white"
    )
    _get_console().print(
        "• Monitor GPU memory usage during large operations", style="white"
    )
...
def gpu_tune(
    ...
    try:
        ...
        _get_console().print(
            f"❌ Invalid image size format: {image_size}. Use WIDTHxHEIGHT (e.g., 1920x1080)",
            style="red",
        )
        return
    ...
    try:
        ...
        if not torch.cuda.is_available():
            _get_console().print(
                "❌ CUDA not available. GPU tuning requires CUDA-enabled PyTorch.",
                style="red",
            )
            return
        ...
        _get_console().print(
            "❌ PyTorch not available. GPU tuning requires PyTorch.", style="red"
        )
        return
    ...
    if test_images_path:
        if test_images_path.is_dir():
            ...
            _get_console().print(
                f"📁 Using {len(test_image_paths)} images from {test_images_path}",
                style="blue",
            )
        else:
            test_image_paths = [test_images_path]
            _get_console().print(
                f"📁 Using single test image: {test_images_path}", style="blue"
            )

    # Run GPU tuning
    _get_console().print(
        f"🧪 Testing batch sizes from {start_batch_size} to {max_batch_size}",
        style="blue",
    )
    _get_console().print(
        f"🖼️  Image size: {parsed_image_size[0]}x{parsed_image_size[1]}", style="blue"
    )
    ...
    try:
        ...
        _get_console().print(
            f"🎯 Optimal GPU batch size: {optimal_batch_size}", style="green"
        )
        _get_console().print(
            f"💡 You can use this batch size with: --batch-size {optimal_batch_size}",
            style="blue",
        )
        ...
        if optimal_batch_size > 1:
            ...
            _get_console().print(
                f"📈 Estimated performance improvement: ~{improvement}%", style="green"
            )
